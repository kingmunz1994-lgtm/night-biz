/**
 * deploy.ts — Deploy NightBizToken to Midnight Preprod
 *
 * Run:
...
const ZK_CONFIG_PATH = path.resolve(import.meta.dirname, '..', 'contracts', 'managed', 'night-biz-token');
...
async function main() {
  ...
  console.log('\n🚀 Deploying NightBizToken to Midnight Preprod...\n');
  ...
  const compiled = CompiledContract
    .make('night-biz-token', ContractModule.Contract)
  ...
  const providers = {
    ...
    privateStateProvider: levelPrivateStateProvider({ privateStateStoreName: 'night-biz-deploy', walletProvider: ctx.wallet as any }),
  ...
  const deployed = await deployContract(providers, { compiledContract: compiled, privateStateId: 'nightBizState', initialPrivateState: {} });

  console.log(`\n   ✅ NightBizToken deployed!`);
